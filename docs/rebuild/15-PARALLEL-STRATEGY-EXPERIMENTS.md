...
можно было сравнить с контрольным вариантом без смешивания причин:

- перенос stop в cost-adjusted безубыток после `+1R`;
- trailing-stop 3% активируется только после `+1,5R`;
- позиция закрывается по обратному EMA-пересечению на завершённой свече;
- исходные пары, timeframe, входы, риск, SL/TP и cost model не меняются.

Контрольный deployment остаётся без изменений. Новый вариант можно запускать в dry-run
только после passed backtest и walk-forward на том же immutable snapshot. Выводы по
