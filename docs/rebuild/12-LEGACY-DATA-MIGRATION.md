...
Перед реальным импортом выбрать один из вариантов:

1. оставить всю legacy-историю только в архиве и начать каноническую историю с чистой БД;
2. вручную перенести одну выбранную стратегию как новый draft с review каждого поля;
3. построить отдельную read-only legacy projection, которая никогда не участвует в
   runtime, validation gates и текущей аналитике.

Рекомендуемый вариант — первый. Он соответствует требованию чистой БД и не смешивает
старые неподтверждённые результаты с новой воспроизводимой моделью.
