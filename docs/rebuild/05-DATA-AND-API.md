...
При открытии Position runtime фиксирует `entryRegime` из соотношения fast/slow EMA и
`entrySession` из UTC-времени свечи. Сессии заданы детерминированно: Asia 00:00–08:00,
Europe 08:00–13:00, US 13:00–21:00, off-hours 21:00–00:00 UTC. Эти поля не
пересчитываются задним числом и копируются в Trade при любом автоматическом или ручном
закрытии. Миграция сохраняет совместимость старых данных через `UNKNOWN`.

`GET /api/v1/analytics` строит проекцию только из канонических Trade. Кроме summary,
equity, drawdown и daily PnL ответ содержит breakdowns по стратегии, паре, причине
выхода, режиму и сессии входа, адаптивную гистограмму Net PnL и фиксированные диапазоны
времени в позиции. Все фильтры применяются до расчёта проекции.

`GET /api/v1/journal` возвращает cursor-paginated записи, последние review sessions,
фильтры и допустимые цели связей. `POST /api/v1/journal/entries` создаёт типизированную
запись и проверяет каждую ссылку по workspace до commit. Полиморфная `JournalLink`
использует отдельные nullable foreign keys, поэтому ссылка имеет referential integrity и
не хранит копию предметных данных в JSON.

`POST /api/v1/journal/reviews` атомарно сохраняет период, итог, выводы, следующие
