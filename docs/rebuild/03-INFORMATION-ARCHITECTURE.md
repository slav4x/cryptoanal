...
- **Обзор** — текущее состояние и главные отклонения.
- **Торговля**
  - Рынки и пары;
  - Сделки.
- **Стратегии**
...
Группа «Система» визуально вторична и может быть свёрнута. Управление runtime доступно
через overview/strategy deployment и не должно становиться отдельным набором несвязанных
кнопок.
