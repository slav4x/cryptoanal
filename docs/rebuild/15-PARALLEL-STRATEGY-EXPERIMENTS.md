...
Актуально на 2026-09-15. Контур остаётся `dry-run`: приватные ордера на Bybit не
отправляются.
...
| EMA Aggressive Managed Exit v2 | BTC, ETH, LINK | 15m | both 30/100, market      | 1%; 3/9%       | 174 сделки; PF 1,40; +2 086,85 | 116 сделок; PF 1,62; +1 516,10 |
...
агрегирует шестнадцать account и начинает со 160 000 USDT.

Набор включает четыре алгоритмические семьи: тринадцать EMA-crossover вариантов и по
одному первичному кандидату breakout, mean-reversion и momentum. Все шестнадцать
...
### EMA Aggressive Both 30-100 Managed Exit

Отдельный кандидат меняет только сопровождение исходной агрессивной EMA, чтобы результат
можно было сравнить с контрольным вариантом без смешивания причин. Первая версия с
безубытком после `+1R`, trailing 3% после `+1,5R` и обратным EMA-выходом была отклонена:
458 сделок, PF 0,87, −1 494,54 USDT и просадка 32,75%. Частый обратный выход разрушил
исходное преимущество и резко увеличил комиссии.

Вторая immutable-версия оставляет входы и базовые SL/TP неизменными, но использует:

- перенос stop в cost-adjusted безубыток после `+0,5R`;
- trailing-stop 4,5% активируется только после `+1,5R`;
- обратный EMA-выход выключен;
- исходные пары, timeframe, входы, риск, SL/TP и cost model не меняются.

V2 прошла backtest: 174 сделки, PF 1,40, +2 086,85 USDT, drawdown 6,49%; и
walk-forward: 116 сделок, PF 1,62, +1 516,10 USDT, drawdown 6,30%. После этого она
одобрена и запущена отдельным dry-run deployment на
`cryptoanal-execution@0.3.0`. Контрольный deployment остаётся без изменений. Выводы по
runtime не делаются до 30 закрытых сделок; полноценное сравнение начинается со 100.
...
- Managed-exit v1 с обратным EMA-выходом дал 458 сделок, PF 0,87, −1 494,54 USDT и
  drawdown 32,75%; версия сохранена как отрицательный результат и не развёрнута.
