...
- [x] Сохранять последовательность полученных котировок и прогресс обработки позиции;
      исключить потерю SL/TP при нескольких котировках за один цикл.
- [x] Восстанавливать открытые позиции после простоя по журналу и минутным свечам;
      блокировать новые входы при неподтверждённой истории.
- [x] Добавить независимый риск-контроль: плавающий убыток и комиссии, общий лимит
      экспозиции аккаунта, атомарный допуск входов, постоянный аварийный выключатель.
- [x] Проверить разрывы потока, рестарт, конкуренцию, лимиты и обновить документацию.

- [x] Применить миграцию P1 и обновить API, worker и dashboard; проверить поток
      и прогресс 12 открытых позиций после backup/restore-check.

Bybit Demo execution остаётся отдельным этапом по решению пользователя.

## Проверка торгового движка — 19 сентября 2026

- [x] Проверить сигналы, сопровождение, сохранение состояния, риск и восстановление.
- [x] Исправить сохранение защиты, устаревшие записи, цену signal exit, финализацию
      свечей, connection gates, учёт entry fees и уникальность worker identity.
- [x] Добавить регрессионные проверки, включая изолированную PostgreSQL.
- [x] Выполнить итоговые проверки проекта и задокументировать ограничения готовности к Demo/Live.
- [x] Обновить работающий worker: backup с проверкой восстановления, две миграции,
      точечный перезапуск и проверка сопровождения открытых позиций.
- [x] Реализовать bounded recovery открытых позиций по журналу, минутам и сигналам;
      неоднозначные пропуски помечать консервативным recovery-exit.
- [x] Заменить защитный latest-ticker путь на сохраняемую последовательность public trades.
- [ ] Расширить golden fixtures на breakout, mean-reversion и EMA, включая gaps,
      комиссии, intrabar limit fill и переход суток.

Подробности: [аудит движка](docs/runtime-audit-2026-09-19.md).
...
- Сократить стартовый JS dashboard до бюджета 100 KiB gzip: превышение было до P1
  (предыдущая развёрнутая сборка — около 129 KiB, текущая — 129.62 KiB).
